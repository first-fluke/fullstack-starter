locals {
  ...
  api_public_url = var.domain != "" ? "https://${var.api_subdomain}.${var.domain}" : var.API_PUBLIC_URL
  web_public_url = var.domain != "" ? "https://${var.domain}" : var.WEB_PUBLIC_URL
  web_rp_id      = var.domain != "" ? var.domain : var.WEBAUTHN_RP_ID
}
